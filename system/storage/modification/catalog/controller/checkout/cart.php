...
// Validate minimum cart Total Amount .
...
// GO TO CART start
                if($this->config->get('go_to_cart_status')==1){
                    ...
                    foreach ($this->cart->getProducts() as $product) {
                        if($product['product_id']==$this->request->post['product_id']){
                            ...
                            if(!empty($product['option'])){
                                foreach ($product['option'] as $cart_options) {
                                    if ($option[$cart_options['product_option_id']]==$cart_options['product_option_value_id']) {
                                        $json['option'] = $cart_options['product_option_value_id'];
                                    }
                                }
                            }
                        }
...
// GO TO CART start
            ...
            foreach($products_cart as $product_cart){
                if(version_compare(VERSION, '2.1', '<') ? $product_cart['key'] : $product_cart['cart_id']==$this->request->post['key']){
                    $json['product_id']=$product_cart['product_id'];
                    $json['minimum']=$product_cart['minimum'];
                    $json['button_cart'] = $this->language->get('button_cart');
                    $json['text_loading'] = $this->language->get('text_loading');
                    if(!empty($product_cart['option'])){
                        foreach ($product_cart['option'] as $option) {
                            $json['product_option_value_id']=$option['product_option_value_id'];
                        }
                    }
                }
            }
