...
class Cart {
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		  ...
		  if ($this->getPcConfig('Enabled') == 'yes' && empty($_GET['token']) && strpos($this->getPcConfig('Method'),'cookies') !== false) {
			setcookie("pc_cookie", $this->session->getId(), strtotime('+'.$this->getPcConfig('Limit').' days'), '/');
		  }
		...
		if (!$query->row['total']) {
			$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW(), visitor_ip='" . $this->getVisitorIp()."'");
		} else {
