<?php
class ControllerModuleInvoiceManager extends Controller {
	private $error = array();
	private $ssl = 'SSL';
	private $tpl = '.tpl';
	...
	public function index() {
		$this->checkfield();
		$this->load->language('module/invoice_manager');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($this->request->post['stay']==1){
				$this->response->redirect($this->url->link('module/invoice_manager', '&store_id='.$data['store_id'].'&token=' . $this->session->data['token'] ,  'SSL'));
			}else{
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/invoice_manager', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('module/invoice_manager', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('module/invoice_manager.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/invoice_manager')) {
